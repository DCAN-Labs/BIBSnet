...
def test_correct_chirality_correct_right_label():
    ...
    correct_right_label = 'Right-Cerebral-White-Matter'
    test_correct_labels(correct_right_label, correctly_labeled_right, nifti_input_file_path, nifti_output_file_path,
                        segment_lookup_table)


def test_correct_labels(correct_label, correctly_labeled, nifti_input_file_path, nifti_output_file_path,
                        segment_lookup_table):
    input_data = get_input_data(nifti_input_file_path)
    free_surfer_label_to_region = get_id_to_region_mapping(segment_lookup_table)
    output_data = get_output_data(nifti_output_file_path)
    assert correct_label == free_surfer_label_to_region[
        input_data[correctly_labeled[0]][correctly_labeled[1]][correctly_labeled[2]]]
    assert correct_label == free_surfer_label_to_region[
        output_data[correctly_labeled[0]][correctly_labeled[1]][correctly_labeled[2]]]
...
def test_correct_chirality_correct_left_label():
    ...
    correct_chirality(nifti_input_file_path, segment_lookup_table, left_right_mask_nifti_file,
                      nifti_output_file_path)
    correctly_labeled_left = (109, 96, 90)
    correct_left_label = 'Left-Cerebral-White-Matter'
    test_correct_labels(correct_left_label, correctly_labeled_left, nifti_input_file_path, nifti_output_file_path,
                        segment_lookup_table)
...
if __name__ == "__main__":
    ...
    test_correct_chirality_correct_left_label()
